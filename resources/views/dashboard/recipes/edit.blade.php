<x-layout>
    ...
    <section class="w-full md:w-4/6 p-6 mt-5 m-auto bg-white rounded-md shadow-md dark:bg-gray-800">
        <h2 class="text-lg font-semibold text-gray-700 capitalize dark:text-white">Update Recipe</h2>
        
        <form action="{{ route('dashboard.recipes.update', $recipe) }}" method="POST">
        @csrf
        @method('PUT')
            <div id="inputs" class="grid grid-cols-1 gap-2 mt-4">
                <div>
                    <label class="text-gray-700 dark:text-gray-200" for="title">Title</label>
                    <input name="title" type="text" value="{{ $recipe->title }}" required minlength="10" maxlength="50"
                ...
                <div>
                    ...
                    <textarea required name="overview" minlength="30" maxlength="150" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">{{ $recipe->overview }}</textarea>
                </div>
                ...
                <div>
                    ...
                    value="@foreach($recipe->ingredients as $ingredient)@if($loop->last){{ $ingredient}}@else{{ $ingredient.',' }}@endif @endforeach" 
                    required minlength="3"
                    class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
                </div>
                @foreach ($recipe->paragraphs as $paragraph)
                <div id="{{ 'paragraph_'.$loop->iteration }}">
                    <label class="text-gray-700 dark:text-gray-200" for="{{ 'paragraph_'.$loop->iteration }}">
                        Paragraph {{ $loop->iteration }}
                        <p class="text-xs text-gray-400 flex">
                            Each paragraph is limited to 2000 characters, add more paragraph if needed
                        </p>
                    </label>
                    <textarea name="{{ 'paragraph_'.$loop->iteration }}" minlength="50" maxlength="2000" required class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">{{ $paragraph }}</textarea>
                </div>
                @endforeach
            </div>
            ...
            <div class="flex justify-between mt-6">
                ...
                <button type="submit" class="px-6 py-2 leading-5 text-white transition-colors duration-200 transform bg-gray-700 rounded-md hover:bg-gray-600 focus:outline-none focus:bg-gray-600">Update Recipe</button>
            </div>
        </form>
    </section>
</x-layout>
